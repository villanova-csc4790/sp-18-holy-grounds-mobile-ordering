...
export default class CappuccinoInfo extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      selectedColor: defColor,import React, { Component } from 'react';
import { Image, Dimensions, TouchableWithoutFeedback, AsyncStorage, StyleSheet } from 'react-native';
import { View, Container, Content, Button, Left, Right, Icon, Picker, Item, Grid, Col, Toast, Text as NBText } from 'native-base';
import { Actions } from 'react-native-router-flux';
import Carousel, { Pagination } from 'react-native-snap-carousel';
  ...
  render() {
    ...
    return(
      <Container style={{backgroundColor: '#fdfdfd'}}>
        ...
        <Content>
            <Image source={require('./Hot Drinks/Cappuccino.jpg')} style={styles.image} />
...
price() {
  ...
  if (this.state.selectedColor != 'None'){
      if (this.state.selectedSize == 'Short - 8 oz'){
        value = this.state.quantity * (3.30 + extras);
      }
      if (this.state.selectedSize == 'Small - 12 oz'){
        value = this.state.quantity * (4.90 + extras);
      }
      if (this.state.selectedSize == 'Medium - 16 oz'){
        value = this.state.quantity * (5.60 + extras);
      }
      if (this.state.selectedSize == 'Large - 24 oz'){
      value = this.state.quantity * (6.00 + extras);
      }
   }
   if (this.state.selectedColor == 'None')  {
     if (this.state.selectedSize == 'Short - 8 oz'){
        value = this.state.quantity * (2.55 + extras);
    }
    if (this.state.selectedSize == 'Small - 12 oz'){
        value = this.state.quantity * (3 + extras);
    }
    if (this.state.selectedSize == 'Medium - 16 oz'){
        value = this.state.quantity * (3.60 + extras);
    }
    if (this.state.selectedSize == 'Large - 24 oz'){
      value = this.state.quantity * (3.95 + extras);
    }
   }
