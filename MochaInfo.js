...
export default class MochaInfo extends Component {
  ...
  renderMilk() {
    let milks = [];
    this.state.product.milks.map((m, i) => {
      milks.push(
        <Item key={i} label={m} value={m} />
      );
    });
    return milks;
  }
...
price() {
  ...
  if (this.state.selectedColor != 'None'){
      ...
      if (this.state.selectedSize == 'Small - 12 oz'){
        value = this.state.quantity *(4.20 + extras);
      }
      if (this.state.selectedSize == 'Medium - 16 oz'){
        value = this.state.quantity * (4.85 + extras);
      }
      if (this.state.selectedSize == 'Large - 24 oz'){
      value = this.state.quantity * (5.52 + extras);
      }
   }
   if (this.state.selectedColor == 'None')  {
     if (this.state.selectedSize == 'Short - 8 oz'){
    ...
    if (this.state.selectedSize == 'Small - 12 oz'){
        value = this.state.quantity * (3.45 + extras);
    }
    if (this.state.selectedSize == 'Medium - 16 oz'){
